# InceptionV3 implementation from jax-fid (https://github.com/matthias-wright/jax-fid)
...
class Dense(nn.Module):
    ...
    def __call__(self, x):
        x = nn.Dense(
            ...
            kernel_init=(
                self.kernel_init if self.params_dict is None else lambda *_: jnp.array(self.params_dict["kernel"])
            ),
...
class BasicConv2d(nn.Module):
    ...
    def __call__(self, x, train=True):
        x = nn.Conv(
            ...
            bias_init=(
                self.bias_init if self.params_dict is None else lambda *_: jnp.array(self.params_dict["conv"]["bias"])
            ),
            dtype=self.dtype,
        )(x)
        if self.params_dict is None:
...
def pool(inputs, init, reduce_fn, window_shape, strides, padding):
    ...
    if not isinstance(padding, str):
        ...
        assert len(padding) == len(window_shape), (
            f"padding {padding} must specify pads for same number of dims as " f"window_shape {window_shape}"
        )
